...
export const usePopupMovable = () => {
    const map = useMap();

    useEffect(() => {
        map.popupMovable.enable();

        return () => {   
            map.popupMovable.disable();
            // if (typeof map.popupMovable.disable === 'function') {
            //     // console.log('Disabling popupMovable');
            //     // console.log(map);
            //     //map.popupMovable.setOptions({movable:false});

            //     //de todas las opcionoes, ninguna logra desactivar el popup movable.

            //     map.eachLayer((layer) => {
            //         console.log (layer);
            //         if (layer instanceof L.Popup) {
            //             layer.options.popupmovable = false;
                        
            //             // Deshabilitamos el popupmovable al crear el popup
            //         }
            //     });
            // } else {
            //     console.error('map.popupMovable.disable is not a function');
            // }
        };
    }, [map]);
};
